  ...
  describe("POST /topics/:topicID/flairs/create", () => {
    it("should create a new flair and redirect", (done) => {
      ...
      request.post(options,
        (err, res, body) => {
          Flair.findOne({where: {name: "Hotness"}})
          .then((flair) => {
            expect(flair).not.toBeNull();
            expect(flair.name).toBe("Hotness");
  ...
  describe("GET /topics/:topicId/flairs/:id", () => {
    it("should render a view with the selected flair", (done) => {
      request.get(`${base}/${this.topic.id}/flairs/${this.flair.id}`, (err, res, body) => {
        expect(err).toBeNull();
        expect(this.flair.color).toContain("blue");
        done();
      });
    });
  });
  ...
  describe("POST /topics/:topicId/flairs/:id/update", () => {
    ...
    it("should update the flair with the given values", (done) => {
      ...
      request.post(options,
        ...
        .then((flair) => {
          expect(flair.name).toBe("Competitions");
          done();
        });
      });  
    });
